...
3. 사용자 환경변수에 development 동기화 대상이 이미 등록되어 있을 때만 동기화를 사용합니다. URL과 키를 명령줄이나 저장소에 넣지 않습니다.
...
수집기는 DOM 텍스트를 우선 사용하고 로그인·CAPTCHA·추가 인증 화면에서는 중단합니다. 모든 결과는 마스킹 및 중복 판정 후 `output/`에 저장되며 쇼핑몰 답변 입력·전송·상태 변경은 수행하지 않습니다.
