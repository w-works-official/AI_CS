...
type SourceSnapshot = { dataUrl: string; width: number; height: number; capturedAt: string; redactionState: 'MASKED_DOM' };
...
type DetailPayload = { case?: RawRow; snapshot?: RawRow | null; messages?: RawRow[]; attachments?: RawRow[]; drafts?: RawRow[]; decisions?: RawRow[]; summary?: unknown; templates?: unknown; library_examples?: unknown; learning_candidates?: unknown };
...
function safeSnapshotDataUrl(value: unknown) {
  const raw = text(value);
  if (!raw.startsWith('data:image/jpeg;base64,') || raw.length > 460_000) return '';
  return /^[A-Za-z0-9+/]+={0,2}$/.test(raw.slice('data:image/jpeg;base64,'.length)) ? raw : '';
}
...
function hydrateCase(row: RawRow, messageRows: RawRow[], attachmentRows: RawRow[], draftRows: RawRow[], decisionRows: RawRow[] = [], summary?: unknown, templates?: unknown, libraryExamples?: unknown, learningCandidates?: unknown, snapshot?: RawRow | null): CsCase {
  const item = baseCase(row);
  const snapshotDataUrl = safeSnapshotDataUrl(snapshot?.data_url);
  if (snapshotDataUrl && text(snapshot?.redaction_state).toUpperCase() === 'MASKED_DOM') {
    item.sourceSnapshot = {
      dataUrl: snapshotDataUrl,
      width: Math.max(1, Number(snapshot?.width ?? 1) || 1),
      height: Math.max(1, Number(snapshot?.height ?? 1) || 1),
      capturedAt: formatDate(snapshot?.captured_at),
      redactionState: 'MASKED_DOM',
    };
  }
...
function hydrateDetailPayload(payload: DetailPayload): CsCase | null {
  ...
  return hydrateCase(payload.case, payload.messages ?? [], payload.attachments ?? [], payload.drafts ?? [], payload.decisions ?? [], payload.summary, payload.templates, payload.library_examples, payload.learning_candidates, payload.snapshot);
}
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <div className="desk-grid">
        ...
        <section className="conversation-column" aria-label="전체 대화">{!selected ? <div className="panel-empty"><span>⌁</span><strong>표시할 문의가 없습니다.</strong><p>상태 필터를 바꾸거나 데이터를 다시 불러와 주세요.</p></div> : <>
          <header className="case-header"><div className="case-header-copy"><div className="case-heading-line"><span className={`status-pill ${statusMeta[selected.status].tone}`}>{statusMeta[selected.status].label}</span><span className={`surface-label ${selected.surface}`}>{selected.surface === 'chat' ? '● 채팅형 문의' : '▤ 게시글형 문의'}</span><span className="case-id">{selected.id}</span></div><div className="case-product-heading">{selected.productThumbnailUrl && <img src={selected.productThumbnailUrl} alt="" referrerPolicy="no-referrer"/>}<div><h2>{selected.product}</h2><p>{selected.productId ? `상품 ID ${selected.productId} · ` : ''}{selected.channel} · 고객 {selected.customer}</p></div></div><div className="case-link-meta"><span>{selected.imageCount > 0 ? `첨부 이미지 ${selected.imageCount}개` : '첨부 이미지 없음'}</span>{selected.sourceSnapshot ? <span>마스킹 캡처 저장됨</span> : <span>캡처 미수집</span>}</div></div><div className="case-header-actions"><button className="icon-button" onClick={() => setSourceSnapshotOpen(true)}>문의 화면 보기</button>{selected.productUrl && <button className="icon-button product-link-button" onClick={() => openExternal(selected.productUrl, '상품 페이지')}>상품 보기 ↗</button>}</div></header>
    ...
    {sourceSnapshotOpen && selected && <div className="source-guide-backdrop snapshot-backdrop" role="presentation" onMouseDown={() => setSourceSnapshotOpen(false)}><section className="source-snapshot-dialog" role="dialog" aria-modal="true" aria-labelledby="source-snapshot-title" onMouseDown={(event) => event.stopPropagation()}><div className="source-guide-heading"><div><span className="source-guide-kicker">마스킹된 수집 화면</span><h2 id="source-snapshot-title">{selected.product}</h2></div><button className="dialog-close" type="button" aria-label="문의 화면 닫기" onClick={() => setSourceSnapshotOpen(false)}>×</button></div><div className="snapshot-meta"><span>{selected.channel}</span><span>{selected.surface === 'chat' ? '채팅형' : '게시글형'}</span><span>고객 {selected.customer}</span><span>{selected.sourceSnapshot?.capturedAt ?? selected.updatedAt}</span></div>{selected.sourceSnapshot ? <div className="captured-screen-wrap"><img className="captured-screen" src={selected.sourceSnapshot.dataUrl} width={selected.sourceSnapshot.width} height={selected.sourceSnapshot.height} alt="개인정보를 마스킹한 문의 화면 캡처"/><p>수집 시점 문의 영역만 저장했으며 텍스트 개인정보는 마스킹되었습니다.</p></div> : <div className="snapshot-conversation"><div className="collection-gap"><span>⌁</span><strong>저장된 문의 화면 캡처가 없습니다.</strong><p>다음 순회 수집에서 캡처가 생성됩니다. 현재 수집된 텍스트는 본문 영역에서 확인할 수 있습니다.</p></div></div>}<div className="snapshot-actions"><span>{selected.sourceSnapshot ? '마스킹 캡처' : '캡처 대기'}</span><div>{selected.sourceUrl && <button className="secondary-button" onClick={() => selected.sourceUrlKind === 'LIST' ? setSourceGuide({ url: selected.sourceUrl, reference: selected.sourceReference, product: selected.product }) : openExternal(selected.sourceUrl, '판매자센터 원문')}>판매자센터 확인 ↗</button>}</div></div></section></div>}
