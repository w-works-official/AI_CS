...
function baseCase(row: RawRow): CsCase {
  ...
  return {
    ...
    updatedRaw: text(row.last_seen_at ?? row.last_changed_at ?? row.last_message_at), status, sourceUrl: text(row.source_url), bodyCollected: Boolean(rawPreview),
...
function hydrateCase(row: RawRow, messageRows: RawRow[], draftRows: RawRow[]): CsCase {
  ...
  if (draft && text(draft.draft_text)) {
    ...
    item.ai = { text: text(draft.draft_text), reason: draftReason(draft.required_checks), generatedAt: formatDate(draft.generated_at), risk: riskLevel(draft.pii_scan) };
  }
...
export default function Home() {
  ...
  const saveReview = async (draftState: 'APPROVED' | 'REJECTED') => {
    if (!selected?.draftId || !localReviewEnabled || reviewSaving) return;
