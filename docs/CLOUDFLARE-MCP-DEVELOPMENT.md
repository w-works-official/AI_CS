...
- no `AI_CS_PROD_*` variable is configured;
- CPU limit is 10 ms;
...
To run locally, copy `.dev.vars.example` to ignored `.dev.vars`, use synthetic or approved test-only values, replace the placeholder public URLs in a local uncommitted configuration, and run:

```powershell
pnpm dev:mcp-worker
```

Never use an operating Sheet, production Apps Script URL, marketplace cookie, browser token, or raw customer data in the local Worker test.
...
After approval, the user enters these secrets in the Cloudflare Dashboard or an interactive hidden prompt:
...
1. `GET /healthz` returns `development`, `upstream_checked=false`, `auto_send=false`, and `marketplace_write_actions=0`.
