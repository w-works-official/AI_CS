...
- Customer-uploaded attachment bytes, cookies, and signed session URLs must never be persisted. Keep only allowlisted HTTPS image metadata; use `SESSION_REQUIRED` when a safe reusable URL is unavailable. A separate `source_snapshot` is allowed only when the macro clones the inquiry DOM, masks known identity/contact/order/address text before capture, emits bounded JPEG data, and marks it `MASKED_DOM`.
...
2. Build a fixed channel cycle in this order: `comments`, `customer_qna`, `customer_center`, `talktalk`, `zigzag_order_inquiry`, `zigzag_item_question`, `ably_inquiry`. Run the existing macro once per requested channel with only that channel in `CS_CHANNELS`, `CS_COLLECTION_PROFILE=operational`, `CS_RUN_MODE=collect_and_reconcile`, `CS_SYNC_MODE=prepare`, and `CS_KEEP_MASKED_OUTPUT=1`. Operational mode uses a rolling 7-day list window, has no whole-cycle deadline, reads the development case index first, and opens chat details only for new, changed-preview, unread, `NEEDS_REPLY`, or `REVIEW` candidates. Individual navigation and selector waits remain finite so one broken page cannot hang forever. Completed unchanged chat rooms are skipped. Use `CS_COLLECTION_PROFILE=backfill` only for a separately requested historical batch; its default range is 30 days and it must never be silently substituted for a normal run. The macro must not sync yet. A collection-only run with no drafts may use direct `sync` only when the user explicitly requests it.
3. Check that only the requested channels have `attempted=true`. Verify visible totals, verified zero states, source keys, content hashes, PII scan, counts, image metadata, and `marketplace_write_actions=0`. Require `operational_refresh.ready=true` before synchronization. Operational candidate scans must report `history_scan_complete=false` with `CANDIDATE_DETAIL_SCAN`; absence from that scan is never a completion signal. Only an explicit count-matched backfill history scan may require every same-window D1 case to reappear. Channels whose pagination cannot prove completeness remain warnings and cannot reconcile missing cases. For TalkTalk, remove the exact known customer-label prefix from list previews and require the normalized record to contain no remaining exact `customer_name`; mask any occurrence in messages or seller replies before `PASS`. Never guess arbitrary Korean text to be a name. A channel may reconcile old cases only when `open_queue_complete=true`, `open_queue_visible_total=open_queue_observed_count`, and the snapshot declares its window. An incomplete or failed list must never close a stored case.
...
11. Call `syncReport(finalReport, config)` exactly once for the completed channel, then checkpoint that channel before advancing to the next one. This is the sole development D1 write for that channel; do not call Apps Script and do not perform a second shadow sync. `syncReport` may internally split a screenshot-heavy payload into bounded idempotent parts, with reconciliation metadata enabled only on the final part. The D1 URL is fixed to the approved development Worker and the hidden key is sent only in the `X-CS-Sync-Key` header. Do not retry browser collection when sync fails. Reusing the same part run IDs must be idempotent. For complete unanswered snapshots, one consecutive absence changes an old open case to `REVIEW`; two consecutive complete-snapshot absences change it to `CLOSED`. A later reappearance reopens it as `NEEDS_REPLY`. Preserve the original case and messages for training and audit.
...
13. If a channel encounters an unsupported selector, login challenge, CAPTCHA, or incomplete detail, do not improvise or patch during the run. Mark that channel failed with the stable error, leave it unsynchronized, continue to the next independent channel, and report the failure immediately in an interactive run. Return a short Korean cycle report with per-channel duration, collected count, new/changed/unchanged/reply-needed counts, draft count, skip reasons, screenshot count, PII result, sync result, and prohibited write count.

## Cycle versus recurring watch

A manual skill invocation performs one complete channel cycle and exits. “Watch” means a Codex heartbeat invokes the same finite cycle again on schedule; it is not an infinite Node process. This keeps AI generation inside Codex without a separate model key and lets a stopped or logged-out browser produce a visible failed cycle instead of a hidden resident process. Never add an overall elapsed-time kill switch. Prevent overlap with the existing collector lock and skip a scheduled cycle when the previous one still holds the lock.
...
var cycleCore = await import("C:/Users/hihi0/.codex/skills/marketplace-cs-monitor/scripts/cycle-core.mjs");
```
